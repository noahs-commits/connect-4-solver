...
impl StrongSolverPlayer{
    pub fn new()->Self{
        StrongSolverPlayer(Table::new())
    }
    pub fn from_file()->Self{
        StrongSolverPlayer(Table::load_from_file())
    }
}

impl Player for StrongSolverPlayer{
...
pub fn ai(game: &Game,cashe: &mut Table) -> u8 {
    ...
    for col in SEARCH_ORDER {
        ...
        match copy.place(col) {
            PlaceOutput::Ok => {
                let new_score = -score(copy,cashe, 0, -beta, -alpha,&mut call_count);
